...
def swift_download_uncompressed(obsid: str, tname:str, dtype: str, dest_dir: pathlib.Path = None) -> None: 
    ...
    if(os.path.isdir(f'{dest_dir}/{tname}/{obsid}')): # if there is already data for the given obsid in dest_dir
        
        # Move the dtype folder's parent to be the current obsid folder
        # ie: dest_dir/obsid/dtype/... -> dest_dir/conventional_name/obsid/dtype/...
