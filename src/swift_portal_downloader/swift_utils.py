...
def remove_incomplete_downloads(
    download_path: pathlib.Path, working_path: pathlib.Path
) -> None:
...
def print_name_scheme(name_scheme: dict) -> None:
    ...
    table = Table(title="")
    table.add_column("Swift Target Name", style="cyan")
    table.add_column("Canonical Name", style="bright_green")

    # Adds all table elements
    for swift_name, canonical_name in name_scheme.items():
        table.add_row(swift_name, canonical_name)
    print()
...
# The entry will be as follows, 'swift_target_name': 'canonical_name'
def manual_add_name(
    swift_target_name: str,
    canonical_name: str,
    name_scheme: dict,
...
def manual_remove_name(
    swift_name: str, name_scheme: dict, name_scheme_path: pathlib.Path
) -> None:
...
def dump_dataframe(
    tlist: str, output_path: pathlib.Path, working_path: pathlib.Path
) -> None:
    console = Console()
    if output_path == working_path:
        console.print(
            f"Found [bold][cyan]{len(tlist)}[/][/] [bright_white]target id(s)[/] from the portal.\nDumping search results to [bold][magenta]portal_search_results.csv[/][/] to current directory."
        )
    else:
        console.print(
    ...
    df.columns = ["Observation id(s)", "Canonical name"]
